...
if(depzt.gt.hzttab(ips,izttab)) then
	write(*,*)' depzt=',depzt,' hzttab(ips,izttab)=',hzttab(ips,izttab)
	write(*,*)' The maximum depth in table is smaller than the focal depth of the present event'
	write(*,*)' In MAJOR_PARAM.DAT correct the 2rd line in REF_PARAM '
	stop
end if
...
3 istep=istep+1
	...
	if(distkm.gt.etab(ips,izz,nn2)) then
		write(*,*)' distkm=',distkm,' etab(ips,izz,nn2)=',etab(ips,izz,nn2)
		write(*,*)' The maximum epicentral distance in table is smaller than the lenght of the present ray'
		write(*,*)' In MAJOR_PARAM.DAT correct the 3rd line in REF_PARAM '
		stop
	end if
